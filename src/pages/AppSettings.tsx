...
export function AppSettings(props) {
	...
	return (
		<Surface className="mx-auto h-full w-full my-auto p-5 rounded-xl">
			<Portal>
				<React.Fragment>
					<Dialog
						dismissable={true}
			...
			<List.Section className="my-auto" title={Translated("settings")}>
				<List.Accordion title="Visuals" left={(props) => <List.Icon className="pl-5" icon="view-quilt" />}>
					...
					/>
					<List.Item disabled title={`Theme: ${FKart.GET_THEME()}`} className="pl-10" left={(props) => <List.Icon icon="theme-light-dark" />}></List.Item>
				</List.Accordion>
				<List.Accordion title="Danger Zone" left={(props) => <List.Icon className="pl-5" icon="alert" />}>
					<List.Item
						...
						onPress={async () => {
							FKart.SET_DATA("testing", {
								...await FKart.GET_DATA("testing"),
